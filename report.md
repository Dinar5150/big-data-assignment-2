...
For storage, I used `store_index.sh` and `store_index.py`. The program connects to the Cassandra container and creates the keyspace `search_engine`. Inside this keyspace, the project stores four tables: `vocabulary`, `postings`, `documents`, and `corpus_stats`. The vocabulary table stores document frequency for each term. The postings table stores term frequency and document information. The documents table stores document title and length. The corpus stats table stores values like `total_docs` and `avg_doc_length`.

For search, I used `search.sh` and `query.py`. The query is tokenized with the same regular expression used during indexing. Then the program reads the needed rows from Cassandra and calculates BM25 scores with PySpark. The BM25 formula uses term frequency, document frequency, document length, and average document length. After scoring, the top 10 results are printed in the format `doc_id<TAB>title`.
...
Before running the command, `a.parquet` should be inside `app/a.parquet`.
...
Fullscreen screenshot placeholders for the final submission:

![Fullscreen screenshot of successful indexing](screenshots/indexing-success.png)

This screenshot should show the terminal after the index is created successfully, including the `/indexer` folders and the corpus statistics output.

![Fullscreen screenshot of search query 1](screenshots/query1.png)

This screenshot should show a successful query run for `computer science` with the returned document ids and titles.

![Fullscreen screenshot of search query 2](screenshots/query2.png)

This screenshot should show a successful query run for `history` with the ranked results.
